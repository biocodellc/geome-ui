...
const openTemplate = `<label class='control-label col-xs-3 text-capitalize'>{{ $ctrl.key }}</label>
     <div class='col-xs-9'>`;

const closeTemplate = `</div>`;

const listTemplate = `${openTemplate}<select
      required name='{{$ctrl.key}}' class='form-control'
      ng-options='list for list in $ctrl.lists'
      ng-model='$ctrl.rule[$ctrl.key]'>
  </select>${closeTemplate}`;

const columnTemplate = `${openTemplate}<select
      required name='{{$ctrl.key}}' class='form-control'
      ng-options='column for column in $ctrl.columns'
      ng-model='$ctrl.rule[$ctrl.key]'>
  </select>${closeTemplate}`;

const columnsTemplate = `${openTemplate}<ui-select required name='{{$ctrl.key}}' ng-model='$ctrl.rule[$ctrl.key]' multiple close-on-select='false'>
  ...
  </ui-select>${closeTemplate}`;

const defaultTemplate = `${openTemplate}<input
      required name='{{$ctrl.key}}' class='form-control'
      type='text'
...
export default angular
  .module('fims.projectConfigRuleMetadata', [])
