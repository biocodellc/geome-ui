...
export default class QueryController {
  ...
  queryJson() {
    ...
    const data = {
      ...
      query: this.queryString.trim().length > 0 ? this.queryString : '*',
    };
      ...
      .then(
        ...
        response => {
          if ((response.status = -1 && !response.data)) {
            this.error =
              'Timed out waiting for response! Try again later. If the problem persists, contact the System Administrator.';
          } else {
            this.error =
              response.data.error ||
  ...
  getExpeditions() {
    this.ExpeditionService.all(this.project.projectId).then(
      response => {
        this.selectedExpeditions = [];
        this.expeditions = [];
        response.data.forEach(expedition =>
          this.expeditions.push(expedition.expeditionCode),
        );
      },
      () => {
