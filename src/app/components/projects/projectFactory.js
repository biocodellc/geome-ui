class ProjectFactory {
  ...
  getProjectsForAdmin() {
    return this.$http.get(this.REST_ROOT + 'projects?admin', { cache: this.PROJECT_CACHE });
  }

  get(projectId) {
    return this.getProjects(true)
      .then(response => response.data.find(p => p.projectId === projectId));
  }
  ...
  getMembers(projectId) {
    return this.$http.get(this.REST_ROOT + 'projects/' + projectId + '/members', { cache: this.MEMBER_CACHE });
  }

  removeMember(projectId, username) {
    this.MEMBER_CACHE.removeAll();
    return this.$http.delete(this.REST_ROOT + 'projects/' + projectId + '/members/' + username);
  }
