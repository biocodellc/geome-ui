...
export function editList($uibTooltip) {
  return {
    restrict: 'A',
    scope: {
      list: '=',
...
export function editableList($compile) {
  return {
    priority: 1001,
    ...
    compile: function (el, attrs) {
      ...
      el.attr('edit-list', "");
      el.attr('edit-popover-template', "'app/components/projects/config/templates/edit-list.html'");
...
function _listController($scope, $uibModal) {
  ...
  function remove() {
    var modal = $uibModal.open({
      template: require('./templates/delete-list-confirmation.html'),
      size: 'md',
